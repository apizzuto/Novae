...
class StackingPlots():
    ...
    def __init__(self, delta_t, catalog, weighting, **kwargs):
        ...
        assert weighting in ['optical', 'gamma'], \
            'Weighting scheme not recognized'
        ...
        src, sample_str = get_sources(
            only_gamma, self.weights, self.delta_t / 86400.)
        self.src = src
    ...
    def initialize_analysis(self):
        ...
        ana_dir = cy.utils.ensure_dir(
            '/data/user/apizzuto/csky_cache/greco_ana')
    ...
    def likelihood_scan(self, n_inj=0., inj_gamma=2.0, truth=False):
        ...
        if truth and n_inj != 0.:
            ...
            tr = cy.get_trial_runner(
                self.conf, ana=self.ana, src=self.src,
                inj_conf={'flux': cy.hyp.PowerLawFlux(inj_gamma)})
        trial = tr.get_one_trial(
            n_sig=n_inj, TRUTH=truth, seed=self.rng_seed,
            poisson=False)
        ...
        if not truth:
            ax1.plot(
                n_inj, inj_gamma, marker="^", color="w",
        ...
        if self.savefigs:
            for ftype in ['pdf', 'png']:
                plt.savefig(
                    self.savepath +
                    f'stacking_llh_scan_{self.sample_str}_delta_t'
                    + f'_{self.delta_t:.2e}_truth_{truth}.{ftype}',
    ...
    def background_distribution(self, ax=None, show=True):
        ...
        ax.semilogy(
            x, norm * bg.pdf(x), lw=1, ls='--',
            label=r'$\chi^2[{:.2f}\mathrm{{dof}},\ \eta={:.3f}]$'.format(
                bg.ndof, bg.eta))
        ...
        if self.savefigs:
            for ftype in ['pdf', 'png']:
                plt.savefig(
                    self.savepath +
                    f'stacking_bg_ts_distribution_{self.sample_str}'
                    + f'_delta_t_{self.delta_t:.2e}.{ftype}',
                    dpi=self.dpi, bbox_inches='tight')
            plt.close()
    ...
    def background_vs_time(self):
        ...
        if self.savefigs:
            for ftype in ['pdf', 'png']:
                plt.savefig(
                    self.savepath +
                    f'stacking_bg_vs_time_{self.sample_str}.{ftype}',
                    dpi=self.dpi, bbox_inches='tight')
            plt.close()
    ...
    def sensitivity_plot(self, gamma=2.0):
        ...
        if self.savefigs:
            for ftype in ['pdf', 'png']:
                plt.savefig(
                    self.savepath +
                    f'stacking_signal_and_bg_ts_dists_{self.sample_str}'
                    + f'_delta_t_{self.delta_t:.2e}.{ftype}',
                    dpi=self.dpi, bbox_inches='tight')
            plt.close()

    def plot_sensitivity_vs_time(self, ax=None, gamma=2.0, in_flux=True,
                                 show=True, with_discovery=False):
        ...
        ax.plot(
            self.all_delta_ts, sens_dict[gamma],
            ls='-', lw=2.)
        if with_discovery:
            disc_lab = ''
            ax.plot(
        ...
        if self.savefigs:
            for ftype in ['pdf', 'png']:
                plt.savefig(
                    self.savepath +
                    f'stacking_sensitivity_vs_time_{self.sample_str}.{ftype}',
                    dpi=self.dpi, bbox_inches='tight')
            plt.close()
    ...
    def fitting_plot(self, gamma=2.0, no_labels=False):
        ...
        for gam in gamma:
            ...
            expect_kw = dict(
                color=self.gam_cols[gam], ls='--', lw=1, zorder=-10)
        ...
        if self.savefigs:
            for ftype in ['pdf', 'png']:
                plt.savefig(
                    self.savepath +
                    f'stacking_fitting_bias_{self.sample_str}_'
    ...
    def find_sens_vs_time(self):
        ...
        if self.all_results is None:
            self.get_all_sens()
        self.sens_vs_time = {gamma: [
                self.all_sensitivity[gamma][t] for t in self.all_delta_ts]
                for gamma in self.spec_ind}
        self.ev_sens_vs_time = {gamma: [
            self.all_event_sensitivity[gamma][t] for t in self.all_delta_ts]
            for gamma in self.spec_ind}
        self.disc_vs_time = {gamma: [
            self.all_discovery[gamma][t] for t in self.all_delta_ts]
            for gamma in self.spec_ind}
        self.ev_disc_vs_time = {gamma: [
            self.all_event_discovery[gamma][t] for t in self.all_delta_ts]
            for gamma in self.spec_ind}
    ...
    def get_differential_sens(self):
        ...
        results = np.load(
            self.trials_base + 'differential_sens/'
            + f'{self.sample_str}_delta_t_{self.delta_t:.2e}_'
            + '.pkl',
            allow_pickle=True)
    ...
    def plot_differential_sens(self, ax=None, color=None, label=None,
        ...
        if self.savefigs:
            for ftype in ['pdf', 'png']:
                plt.savefig(
                    self.savepath +
                    f'differential_sens_stacking_nova_{self.sample_str}_'
                    + f'delta_t_{self.delta_t:.2e}.{ftype}',
                    dpi=self.dpi, bbox_inches='tight')
            plt.close()
    ...
    def __init__(self, **kwargs):
        ...
        all_novae = {name: {
            delta_t: GammaRayNova(
                name, delta_t=delta_t, **kwargs) for delta_t in delta_ts}
    ...
    def sensitivity_vs_time(self, **kwargs):
        ...
        for name in self.names:
            ...
            for delta_t in self.delta_ts:
                try:
                    if not discovery:
                        sens[name].append(
                            self.all_novae[name][delta_t].sensitivity_trials[
                                spec]['E2dNdE'])
                    good_ts[name].append(delta_t)
                    if discovery:
                        disc[name].append(
                            self.all_novae[name][delta_t].discovery_trials[
                                spec]['E2dNdE'])
                except Exception as e:
                    if self.verbose:
                        print(e)
        ...
        for name in sens.keys():
            try:
                if discovery:
                    ax.plot(
                        good_ts[name], disc[name], color=self.sens_col,
                        ls='--')
                else:
                ...
                if 'annotate' in kwargs.keys():
                    if kwargs['annotate'] and len(good_ts[name] > 0):
                        if discovery:
                            plt.annotate(
                                name,
                                (good_ts[name][-1], disc[name][-1]))
                        else:
                            plt.annotate(
                                name,
                                (good_ts[name][-1], sens[name][-1]))
                if discovery:
                    ax.scatter(
                        self.full_time_novae[name].delta_t,
                        self.full_time_novae[name].discovery_trials[
                            spec]['E2dNdE'],
                    ...
                    ax.scatter(
                        self.full_time_novae[name].delta_t,
                        self.full_time_novae[name].sensitivity_trials[
                            spec]['E2dNdE'],
                        marker='*', s=20, color=sns.xkcd_rgb['almost black'],
                        zorder=20)
            except Exception as e:
        ...
        if self.savefigs:
            ...
            for ftype in ['pdf', 'png']:
                plt.savefig(
                    self.savepath +
                    f'{sens_str}_vs_time_gamma_{spec:.1f}.{ftype}',
                    dpi=self.dpi, bbox_inches='tight')

    def background_ts_panel(self, **kwargs):
        if 'fig' not in kwargs.keys() and 'axs' not in kwargs.keys():
            fig, aaxs = plt.subplots(
        ...
        if 'delta_t' in kwargs.keys():
            ...
            for name in self.names:
                ax = axs[ii]
                try:
                    self.all_novae[name][delta_t].background_ts_plot(
                        ax=ax, label_axes=False)
                    used_axs.append(ax)
                    ii += 1
                except Exception as e:
                    print(e)
            ...
            for name in self.names:
                ax = axs[ii]
                try:
                    self.full_time_novae[name].background_ts_plot(
                        ax=ax, label_axes=False)
                    used_axs.append(ax)
                    ii += 1
                except Exception as e:
        ...
        if 'omit_title' not in kwargs.keys():
            ...
            fig.suptitle(title, y=0.92, fontsize=self.fontsize)
        if self.savefigs:
            delta_t_str = f"delta_t_{delta_t:.2e}" if 'delta_t' \
                in kwargs.keys() else 'full_gamma_time'
            for ftype in ['pdf', 'png']:
                plt.savefig(
                    self.savepath +
                    f'background_ts_panel_{delta_t_str}.{ftype}',
                    dpi=self.dpi, bbox_inches='tight')
    ...
    def ns_fitting_panel(self, **kwargs):
        ...
        if 'delta_t' in kwargs.keys():
            ...
            for name in self.names:
                ax = axs[ii]
                try:
                    self.all_novae[name][delta_t].ns_fit_bias_plot(
                        ax=ax, label_axes=False, gamma=spec)
                    used_axs.append(ax)
                    ii += 1
                except Exception as e:
                    pass
            ...
            for name in self.names:
                ax = axs[ii]
                try:
                    self.full_time_novae[name].ns_fit_bias_plot(
                        ax=ax, label_axes=False, gamma=spec)
                    used_axs.append(ax)
                    ii += 1
                except Exception as e:
        ...
        if self.savefigs:
            ...
            for ftype in ['pdf', 'png']:
                plt.savefig(
                    self.savepath +
                    f'ns_fitting_panel_{delta_t_str}_gamma_{spec:.1f}.{ftype}',
                    dpi=self.dpi, bbox_inches='tight')
    ...
    def gamma_fitting_panel(self, **kwargs):
        ...
        if 'delta_t' in kwargs.keys():
            ...
            for name in self.names:
                ax = axs[ii]
                try:
                    self.all_novae[name][delta_t].gamma_fit_bias_plot(
                        ax=ax, label_axes=False)
                    used_axs.append(ax)
                    ii += 1
                except Exception as e:
            ...
            for name in self.names:
                ax = axs[ii]
                try:
                    self.full_time_novae[name].gamma_fit_bias_plot(
                        ax=ax, label_axes=False)
                    used_axs.append(ax)
                    ii += 1
                except Exception as e:
        ...
        if self.savefigs:
            ...
            for ftype in ['pdf', 'png']:
                plt.savefig(
                    self.savepath +
                    f'gamma_fitting_panel_{delta_t_str}.{ftype}',
                    dpi=self.dpi, bbox_inches='tight')

    def compare_photon_to_nus(self, **kwargs):
        if 'fig' not in kwargs.keys() and 'ax' not in kwargs.keys():
            fig, ax = plt.subplots(dpi=self.dpi)
        else:
            fig = kwargs['fig']
            ax = kwargs['ax']
        ...
        if self.savefigs:
            ...
            for ftype in ['pdf', 'png']:
                plt.savefig(
                    self.savepath +
                    f'photon_and_nu_SED_{delta_t_str}.{ftype}',
                    dpi=self.dpi, bbox_inches='tight')
...
class GammaRayNova():
    ...
    def __init__(self, name, **kwargs):
        ...
        if self.full_time:
            ...
            try:
                ...
                print(
                    'Must either enter a time window or use default '
                    + 'full gamma-ray duration')
        self.name = name
        self.spec_ind = kwargs.pop('index', [2., 2.5, 3.0])
        if type(self.spec_ind) is float:
            self.spec_ind = [self.spec_ind]
        ...
        try:
            ...
            for ind in self.spec_ind:
                ...
                tmp_trials_base = self.trials_base \
                    + f'nova_*_{name}_delta_t_{self.delta_t_str}_' \
                    + f'gamma_{ind:.1f}_trials.pkl'
            ...
            if self.verbose:
                print(
                    f"Could not find trials for Nova {self.name} "
                    + "with analysis details:")
                print(f"\t Gamma: {self.spec_ind}")
    ...
    def ns_fit_bias_plot(self, ax=None, label_axes=True, **kwargs):
        ...
        h = hl.hist((self.fitting_trials[gamma].ntrue,
                    ...
                    bins=(ns_bins, 100))
        hl.plot1d(
            ax, h.contain_project(1),
            errorbands=True, drawstyle='default',
            color=self.gamma_colors[gamma])
        lim = [0., max(n_sigs)+1]
        ax.set_xlim(ax.set_ylim(lim))
    ...
    def compare_sens_to_photons(self, ax=None, label_axes=True, **kwargs):
        ...
        for spec in self.spec_ind:
            nu_sens_spec = np.power(ens, -spec)*self.sensitivity_trials[
                spec]['E2dNdE']*1e3*(1e-3)**(2.-spec)
            en_msk = (ens > self.central_90[spec][0]) \
                & (ens < self.central_90[spec][1])
            ax.plot(
                ens[en_msk], nu_sens_spec[en_msk]*ens[en_msk]**2,
                color=self.gamma_colors[spec],
                lw=1.5, zorder=5*(5.-spec), alpha=alpha)
            handles.append(Line2D(
                [0], [0], color=self.gamma_colors[spec],
        ...
        ax.plot(
            ens, photon_flux * ens**2. * self.delta_t,
    ...
    def calc_flux_norm(self, gamma, cutoff, norm):
        ...
        Parameters:
        -----------
            -norm: float, integrated flux above 100 MeV in units of photons
            per cm^-2 s^-1
        Returns:
        --------
        ...
        if np.isinf(cutoff):
            ...
            denom = cutoff ** (1.-gamma)*float(
                mpmath.gammainc(1.-gamma, 0.1/cutoff))
        return norm / denom
...
class GRECOPlots():
    ...
    def __init__(self, **kwargs):
        ...
        self.verbose = kwargs.pop('verbose', False)
        self.low_en_bin = 0.0
        self.log_energy_bins = kwargs.pop(
            'log_e_bins', np.linspace(0.0, 4., 31))
        self.sin_dec_bins = kwargs.pop(
            'sin_dec_bins', np.linspace(-1., 1., 31))
        self.initialize_analysis()
        self.dpi = kwargs.pop('dpi', 150)
        self.savefigs = kwargs.pop('save', False)
        self.savepath = kwargs.pop('output', '/data/user/apizzuto/Nova/plots/')
    ...
    def declination_pdf(self, **kwargs):
        if 'ax' not in kwargs:
            ...
            ax = kwargs['ax']
        hl.plot1d(
            ax, self.ana[0].bg_space_param.h, crosses=True,
            color='k', label='histogram')
        sd = np.linspace(-1, 1, 300)
        ax.plot(
            sd, self.ana[0].bg_space_param(sindec=sd),
            label='spline')
        ax.set_ylim(0)
    ...
    def angular_error_plot(self, **kwargs):
        ...
        ax.plot(
            nans, nans,
            color=color, lw=5, alpha=1 - (1-0.3)**1, label=r'80\%')
        ax.plot(
            nans, nans,
            color=color, lw=5, alpha=1 - (1-0.3)**2, label=r'50\%')
        ax.plot(
            nans, nans,
            color=color, lw=5, alpha=1 - (1-0.3)**3, label=r'20\%')
        ...
        if self.savefigs:
            for ftype in ['pdf', 'png']:
                plt.savefig(
                    self.savepath +
                    f'GRECO_energy_vs_error_true_energy_{true_energy}_' +
                    f'true_errors_{true_error}.{ftype}',
                    dpi=self.dpi, bbox_inches='tight')
    ...
    def effective_area(self, **kwargs):
        ...
        for ii, (low_dec, high_dec) in enumerate(sin_decs):
            ...
            mc_cut = mc[dec_msk]
            denom = mc_cut['true_energy'] * dlog_E[np.digitize(
                np.log10(mc_cut['true_energy']), bins=logE_bins) - 1]
            denom *= 1e4 * d_omega * np.log(10.)
            weights = mc_cut['oneweight'] / denom
            lab = f"{np.degrees(np.arcsin(low_dec)):.1f}" + \
                r'$^{\circ} <  \delta < \; $' \
                + f"{np.degrees(np.arcsin(high_dec)):.1f}" + r'$^{\circ}$'
            ax.hist(
                mc_cut['true_energy'], bins=E_bins,
                weights=weights,
        ...
        ax.set_xlabel(r'$E_{\nu}$ (GeV)', fontsize=20)
        ax.set_ylabel(r'$A_{\mathrm{eff}}$ (m$^{2}$)', fontsize=20)
...
class SynthesisPlots():
    ...
    def gamma_lightcurve_with_greco_rate(self, show_opt=False):
        ...
        for grl, exp in zip(self.grls, self.exps):
            ...
            ts.append(b)
            ut, b = np.histogram(
                grl['start'], bins=time_bins,
                weights=grl['livetime'])
            uptime.append(ut)

        rates = [np.where(up != 0., v / (up * 86400.), 0.) for up, v in zip(
            uptime, vals)]
        val_err = [np.sqrt(v) for v in vals]
        rate_err = [np.where(up != 0., v_err / (up * 86400.), 0.)
                    for up, v_err in zip(uptime, val_err)]

        fig, ax = plt.subplots(figsize=(12, 4), dpi=200)

        for i in range(len(ts)):
            plt.errorbar(
                ts[i], rates[i], yerr=rate_err[i],
        ...
        for i, nova in self.nova_info.iterrows():
            if nova['gamma']:
                if not has_labeled_gam:
                    plt.axvspan(
                        nova['gamma_start'].mjd, nova['gamma_stop'].mjd,
        ...
        # plt.text(
        #     56200, 0.0041,
        #     'IceCube Preliminary', color=sns.xkcd_rgb['tomato red'],
        #     fontsize=20)
        if self.save:
            for ftype in ['pdf', 'png']:
                plt.savefig(
    ...
    def all_sky_scatter_plot(self, fig=None, ax=None,
        ...
        equatorial = SkyCoord(
            ra=self.nova_info['RA'][~gamma_msk]*u.deg,
            dec=self.nova_info['Dec'][~gamma_msk]*u.deg)
        gamma_coords = SkyCoord(
            ra=self.nova_info['RA'][gamma_msk]*u.deg,
            dec=self.nova_info['Dec'][gamma_msk]*u.deg)

        ax.scatter(
            -1*equatorial.ra.wrap_at('360d').radian + np.pi,
            equatorial.dec.radian,
            zorder=20, s=10, c=sns.xkcd_rgb['grey'])
        ax.scatter(
            -1*gamma_coords.ra.wrap_at('360d').radian + np.pi,
            gamma_coords.dec.radian,
            zorder=20, s=14, marker='^',
            c=sns.xkcd_rgb['orange pink'])

        ax.scatter(
            -1.*gplane_icrs.ra.wrap_at('360d').radian + np.pi,
    ...
    def mollview_with_sensitivity(self, **kwargs):
        ...
        cbaxes.axis('off')
        cbar = fig.colorbar(
            cc, ax=cbaxes, orientation='horizontal',
        ...
        ax.legend(
            loc=(0.62, -0.22), handles=legend_els, ncol=1,
            frameon=False)
